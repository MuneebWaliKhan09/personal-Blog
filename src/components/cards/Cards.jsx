    ...
    return (
        <>
            <div className={`flex justify-center  flex-wrap gap-12  my-10 p-4 `}>
                ...
                {
                    vissibleCards.map((d) => (
                        <div key={d.id} className="flex cardsbody  gap-4 flex-col justify-center w-full max-w-[251px] mb-4 pb-1 border border-solid border-2 shadow-2xl  border-grey-600 rounded-xl">
                            <Image width={250} height={250} quality={50} className="rounded-t-[0.3rem] h-[26vh] max-w-sm  hover:transform object-cover" src='/c.jpg' alt="" />
                            <div className="flex flex-col flex-grow gap-3 px-3 pb-3 ">
                                ...
                                <button className=" bg-gray-600 justify-items-end  hover:bg-gray-500 hover:transition transition-all  text-white font-bold py-2 px-4 rounded-full "><Link href={`/blog/${d._id}`}>See more</Link></button>
                            </div>
                        </div>
                    ))
